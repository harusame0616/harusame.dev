...
test("読込中のときにローディングが表示される", () => {
  mocks.queryComments.mockReturnValue(new Promise(() => {}));

  const screen = render(<CommentListContainer articleSlug="test" />);
  const loadingItem = screen.getByRole("listitem", { name: "読込中" });
  expect(loadingItem).toBeVisible();

  mocks.queryComments.mockClear();
});

describe("ネットワークモック", () => {
  const endpoint = "http://127.0.0.1:24321/rest/v1/article";
  beforeAll(async () => {
    const actual = await vi.importActual("../infrastractures/get-comments");
    mocks.queryComments.mockImplementation(
      actual.queryComments as unknown as () => Promise<any>
    );
  });
